...
const referralAdsRequired = 10;
const referralRewardThreshold = 1;
const referralRewardCoins = 10;
...
function makeReferralCode(id) {
  return `FF${String(id || uid()).replace(/[^a-z0-9]/gi, "").slice(0, 8).toUpperCase()}`;
}
...
function createDefaults() {
  ...
  return { activeTournamentId: first.id, currentUserId: null, users: [], transactions: [], walletRequests: [], withdrawRequests: [], referrals: [], payoutDetails: [], tournaments: [first] };
}

function migrateState(saved) {
  if (Array.isArray(saved?.tournaments)) {
    saved.users = Array.isArray(saved.users) ? saved.users : [];
    saved.users = saved.users.map((user) => ({ ...user, referralCode: user.referralCode || makeReferralCode(user.id) }));
    ...
    saved.referrals = Array.isArray(saved.referrals) ? saved.referrals : [];
...
function dbProfile(row) {
  return {
    ...
    referralCode: row.referral_code || makeReferralCode(row.id),
...
function dbReferral(row) {
  return {
    id: row.id,
    referrerId: row.referrer_id,
    referredUserId: row.referred_user_id,
    referredName: row.referred_name,
    watchedAds: Number(row.watched_ads_count || 0),
...
function optionalRows(result) {
  if (result?.error) {
    console.warn("Optional cloud table unavailable", result.error);
    return [];
  }
  return result?.data || [];
}
...
async function cloudCall(label, action) {
  try {
    ...
    showToast(`${label} failed: ${error.message || "check details"}`);
    throw error;
  }
}

async function ensureCloudReferralCode(profile) {
  if (!cloudReady || !profile?.id) return;
  const code = profile.referralCode || makeReferralCode(profile.id);
  profile.referralCode = code;
  const profileResult = await cloud.from("profiles").update({ referral_code: code }).eq("id", profile.id);
  if (profileResult.error) console.warn("Referral code profile update skipped", profileResult.error);
  const codeResult = await cloud.from("referral_codes").upsert({
    user_id: profile.id,
    code,
    name: profile.name || "Player"
  }, { onConflict: "user_id" });
  if (codeResult.error) console.warn("Referral code table update skipped", codeResult.error);
}
...
async function refreshCloudState() {
  ...
  if (authUser) {
    ...
    if (own.data) {
      ...
      await ensureCloudReferralCode(currentProfile);
  ...
  const referralFilter = authUser ? `referrer_id.eq.${authUser.id},referred_user_id.eq.${authUser.id}` : "";
  const [tournamentsResult, squadsResult, roomsResult, walletResult, payoutResult, withdrawResult, referralsResult] = await Promise.all([
    ...
    authUser ? cloud.from("referrals").select("*").or(referralFilter).order("created_at", { ascending: false }) : { data: [], error: null }
  ]);

  const tournamentRows = optionalRows(tournamentsResult);
  const squadRows = optionalRows(squadsResult);
  const roomRows = optionalRows(roomsResult);
  const walletRows = optionalRows(walletResult);
  const payoutRows = optionalRows(payoutResult);
  const withdrawRows = optionalRows(withdrawResult);
  const referralRows = optionalRows(referralsResult);

  const tournaments = tournamentRows.map((row) => dbTournament(row, squadRows, roomRows));
  state = {
    activeTournamentId: state.activeTournamentId,
    currentUserId: authUser?.id || null,
    users,
    transactions: [],
    walletRequests: walletRows.map((row) => ({
      id: row.id,
      userId: row.user_id,
      name: row.name,
      contact: row.contact,
    ...
    withdrawRequests: withdrawRows.map(dbWithdraw),
    referrals: referralRows.map(dbReferral),
    payoutDetails: payoutRows.map(dbPayout),
...
async function syncAndRender() {
  if (cloudReady) {
    try {
      ...
      showToast(`Cloud sync failed: ${error.message || "check Supabase"}`);
    }
  }
  render();
}
...
function activateView(view) {
  ...
  const requestedSection = $(`[data-view="${requested}"]`);
  const target = requestedSection && !requestedSection.classList.contains("removed-view") ? requested : "dashboard";
...
function renderOrganizerMode() {
  ...
  $("#modeLabel").textContent = organizerMode ? "Organizer panel" : cloudReady ? "Supabase connected" : "Supabase not connected";
...
function renderRefer() {
  ...
  $("#referralList").innerHTML = referredByUser.map((item) => `
    <div class="payout-card">
      ...
      <span>Ads watched: ${Math.min(item.watchedAds, referralAdsRequired)} / ${referralAdsRequired}</span>
...
function referralRewardStats(user = currentUser()) {
  const rows = state.referrals.filter((item) => item.referrerId === user?.id && item.completed && !item.rewardGranted);
  return { claimableRows: rows, claimableCount: rows.length };
}
...
async function saveReferralCode(userId, code, name) {
  const result = await cloud.from("referral_codes").upsert({ user_id: userId, code, name }, { onConflict: "user_id" });
...
async function createOrRepairProfile(authUser, fallback = {}) {
  const name = String(fallback.name || authUser.user_metadata?.name || authUser.email?.split("@")[0] || "Player").trim();
  const mobile = String(fallback.mobile || authUser.user_metadata?.mobile || "Not provided").trim();
  const referralCode = fallback.newReferralCode || authUser.user_metadata?.own_referral_code || makeReferralCode(authUser.id);
  const existing = await cloud.from("profiles").select("*").eq("id", authUser.id).maybeSingle();
  if (existing.error) throw existing.error;
  if (!existing.data) {
    const fullInsert = await cloud.from("profiles").insert({
      id: authUser.id,
      name,
      mobile,
      wallet_balance: 0,
    ...
    if (fullInsert.error) {
      const basicInsert = await cloud.from("profiles").insert({
        id: authUser.id,
        name,
        mobile,
        wallet_balance: 0
  ...
  await saveReferralCode(authUser.id, referralCode, name);
  return { name, mobile, referralCode };
}
...
$("#authForm").addEventListener("submit", async (event) => {
  ...
  if (!cloudReady) {
    showToast("Supabase not connected. Check internet and install the latest APK.");
    return;
  }
    ...
    const result = await cloudCall("Register", () => cloud.auth.signUp({
      ...
      options: {
        data: {
          name: data.name.trim(),
          mobile,
          own_referral_code: pendingReferralCode,
          friend_referral_code: referralCode
        }
      }
    }));
    if (!result.data.session) {
      $("#authModal").close();
      showToast("Registered. Check email, or disable email confirmation, then login.");
      return;
    }
...
$("#copyReferralCode").addEventListener("click", () => {
  if (!requireLogin()) return;
  const code = currentUser().referralCode || makeReferralCode(currentUser().id);
  navigator.clipboard.writeText(code);
  showToast("Referral code copied.");
});

$("#claimReferralReward").addEventListener("click", async () => {
  if (!requireLogin()) return;
  const user = currentUser();
  const { claimableRows, claimableCount } = referralRewardStats(user);
  if (claimableCount < referralRewardThreshold) {
